...
class TestHeatPumpState:
    ...
    def test_update_unmatched_demand_is_always_positive(hp_state):
        hp_state.update_unmatched_demand(CURRENT_MARKET_SLOT, -2)
        assert hp_state._unmatched_demand[CURRENT_MARKET_SLOT] == 0
        hp_state.update_unmatched_demand(CURRENT_MARKET_SLOT, 2)
        assert hp_state._unmatched_demand[CURRENT_MARKET_SLOT] == 2
