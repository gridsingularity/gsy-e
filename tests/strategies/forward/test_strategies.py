...
load_parameters = {
    AvailableMarketTypes.INTRADAY: ForwardOrderUpdaterParameters(duration(minutes=5), 10, 40, 20),
    AvailableMarketTypes.DAY_FORWARD: ForwardOrderUpdaterParameters(
        duration(minutes=30), 20, 40, 20
    ),
    AvailableMarketTypes.WEEK_FORWARD: ForwardOrderUpdaterParameters(duration(days=1), 30, 50, 20),
...
pv_parameters = {
    AvailableMarketTypes.INTRADAY: ForwardOrderUpdaterParameters(duration(minutes=5), 10, 8, 20),
    AvailableMarketTypes.DAY_FORWARD: ForwardOrderUpdaterParameters(
        duration(minutes=30), 20, 18, 20
    ),
    AvailableMarketTypes.WEEK_FORWARD: ForwardOrderUpdaterParameters(duration(days=1), 30, 28, 20),
    AvailableMarketTypes.MONTH_FORWARD: ForwardOrderUpdaterParameters(
...
class TestForwardStrategies:
    ...
    @pytest.mark.parametrize(
        "market_type, expected_order_updater_count, next_slot_timestamp, ",
        [
            (AvailableMarketTypes.INTRADAY, 24 * 4 - 1, CURRENT_MARKET_SLOT.add(minutes=15)),
            (AvailableMarketTypes.DAY_FORWARD, 24 * 7 - 1, CURRENT_MARKET_SLOT.add(days=1)),
            (
    ...
    def test_order_updaters_follow_market_slots(
        forward_strategy_fixture, market_type, expected_order_updater_count, next_slot_timestamp
    ):
        ...
        assert (
            list(strategy._order_updaters[market_object].keys()) == market_object.market_time_slots
        )
        assert len(strategy._order_updaters[market_object].keys()) == expected_order_updater_count

        with patch("gsy_e.models.area.Area.now", new_callable=PropertyMock) as now_mock:
            now_mock.return_value = next_slot_timestamp
            area.cycle_markets()
            assert (
                len(strategy._order_updaters[market_object].keys()) == expected_order_updater_count
            )
            assert (
    ...
    def test_forward_strategy_updates_orders_on_tick(self, forward_strategy_fixture, market_type):
        strategy = forward_strategy_fixture[0]
        area = forward_strategy_fixture[1]
        strategy._energy_params.get_available_energy_kWh = MagicMock(return_value=100.0)
        area.activate()
        ...
        with patch("gsy_e.models.area.Area.now", new_callable=PropertyMock) as now_mock:
            ...
            for time_slot, old_order_list in old_orders.items():
                ...
                market_params = area.forward_markets[
                    market_type
                ].get_market_parameters_for_market_slot(time_slot)
                slot_completion_ratio = (
                    updater_params.update_interval.total_minutes()
                    / (
                        market_params.closing_time
                        - updater_params.update_interval
                ...
                if isinstance(strategy, ForwardPVStrategy):
                    assert isclose(
                        ...
                        (
                            ...
                            - slot_completion_ratio
                            * abs(updater_params.initial_rate - updater_params.final_rate)
                        ),
                        abs_tol=0.00009,
                    )
                else:
