...
class TestSettlementMarketStrategy:
    ...
    def _setup_strategy_fixture(self, strategy_fixture, should_post_bid, should_post_offer):
        strategy_fixture.owner = self.area_mock
        strategy_fixture.state.set_energy_measurement_kWh(1, self.time_slot)
        strategy_fixture.state.should_post_bid = MagicMock(return_value=should_post_bid)
        strategy_fixture.state.should_post_offer = MagicMock(return_value=should_post_offer)
    ...
    @pytest.mark.parametrize("should_post_bid", [True, False])
    @pytest.mark.parametrize("should_post_offer", [True, False])
    def test_event_market_cycle_posts_bids_and_offers(
            self, strategy_fixture, should_post_bid, should_post_offer):
        self._setup_strategy_fixture(strategy_fixture, should_post_bid, should_post_offer)
