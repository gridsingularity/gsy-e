...
class FakeMarket:
    def __init__(self, count):
        self.count = count
        self.id = str(count)
        self.trade = Trade('id', 'time', Offer('id', datetime.now(), 11.8, 0.5, 'FakeArea'),
                           'FakeArea', 'buyer'
                           )
        self.created_offers = []
        self.offers = {'id': Offer('id', datetime.now(), 11.8, 0.5, 'FakeArea'),
                       'id2': Offer('id2', datetime.now(), 20, 0.5, 'A'),
                       'id3': Offer('id3', datetime.now(), 20, 1, 'A'),
                       'id4': Offer('id4', datetime.now(), 19, 5.1, 'A')}
        self.bids = {}
    ...
    def sorted_offers(self):
        offers = [
            [Offer('id', datetime.now(), 11.8, 0.5, 'A')],
            [Offer('id2', datetime.now(), 20, 0.5, 'A')],
            [Offer('id3', datetime.now(), 20, 1, 'A')],
            [Offer('id4', datetime.now(), 19, 5.1, 'A')]
        ]
        return offers[self.count]
    ...
    def offer(self, price, energy, seller, original_offer_price=None, seller_origin=None):
        offer = Offer('id', datetime.now(), price, energy, seller, original_offer_price,
...
def test_if_storage_doesnt_buy_above_break_even_point(storage_strategy_test2, area_test2):
    ...
    area_test2.current_market.offers = {'id': Offer('id', datetime.now(), 10.1, 1,
                                                    'FakeArea',
                                                    area_test2.current_market)}
    storage_strategy_test2.event_tick()
    assert len(storage_strategy_test2.accept_offer.calls) == 0
    area_test2.current_market.offers = {'id': Offer('id', datetime.now(), 9.9, 1,
                                                    'FakeArea',
                                                    area_test2.current_market)}
    storage_strategy_test2.event_tick()
    assert len(storage_strategy_test2.accept_offer.calls) == 0
...
def storage_strategy_test5(area_test5, called):
    ...
    area_test5.past_market.offers = {
        'id': Offer('id', datetime.now(), 20, 1, 'A'),
        'id2': Offer('id2', datetime.now(), 20, 3, 'FakeArea'),
        'id3': Offer('id3', datetime.now(), 100, 1, 'FakeArea')
    }
...
def storage_strategy_test7_3(area_test7):
    ...
    s.owner = area_test7
    s.area = area_test7
    s.offers.posted = {Offer('id', datetime.now(),
...
def test_energy_origin(storage_strategy_test15, market_test15):
    ...
    storage_strategy_test15.area.current_market.trade = \
        Trade('id', 'time', Offer('id', datetime.now(), 20, 1.0, 'ChildArea'),
              'ChildArea', 'FakeArea')
    ...
    storage_strategy_test15.area.current_market.trade = \
        Trade('id', 'time', Offer('id', datetime.now(), 20, 2.0, 'FakeArea'), 'FakeArea', 'A')
    storage_strategy_test15.event_trade(market_id=market_test15.id,
                                        trade=storage_strategy_test15.area.current_market.trade)
    assert len(storage_strategy_test15.state.get_used_storage_share) == 2
    assert storage_strategy_test15.state.get_used_storage_share == [EnergyOrigin(
    ...
    storage_strategy_test15.area.current_market.trade = \
        Trade('id', 'time', Offer('id', datetime.now(), 20, 1.0, 'ParentArea'),
              'FakeArea', 'FakeArea')
    storage_strategy_test15.event_trade(market_id=market_test15.id,
                                        trade=storage_strategy_test15.area.current_market.trade)
...
def test_assert_if_trade_rate_is_lower_than_offer_rate(storage_test11):
    market_id = "market_id"
    storage_test11.offers.sold[market_id] = [Offer("offer_id", datetime.now(), 30, 1, "FakeArea")]
