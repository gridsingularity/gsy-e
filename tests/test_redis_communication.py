...
class TestRedisEventDispatching(unittest.TestCase):
    ...
    def test_subscribe(self):
        mock_redis.sub_to_area_event.assert_any_call(
            ...
            self.device2.dispatcher.area_event_dispatcher.area_event_listener_redis)
        mock_redis.sub_to_area_event.assert_any_call(
            f"load/area_event",
            self.device1.dispatcher.area_event_dispatcher.area_event_listener_redis)
        mock_redis.sub_to_area_event.assert_any_call(
            f"area/area_event",
            self.area.dispatcher.area_event_dispatcher.area_event_listener_redis)
    ...
    def test_receive(self):
        ...
        for area_event in [AreaEvent.TICK, AreaEvent.MARKET_CYCLE, AreaEvent.ACTIVATE,
            ...
            self.device1.dispatcher.area_event_dispatcher.area_event_listener_redis(payload)
