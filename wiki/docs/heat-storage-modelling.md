...
Following the heat storage model available heat energy estimation, the electrical energy that has to be produced by the heat pump digital twin is calculated with the following formula implemented in the GSY heat pump digital twin (for more, see the [related GSY Wiki Documentation](heat-pumps-general.md#heat-pumps-and-district-heating){target=_blank}) where the COP calculation generally follows the model proposed by [Ruhnau et al.](https://www.nature.com/articles/s41597-019-0199-y){target=_blank} using the following input parameters (termed here Universal COP model):
