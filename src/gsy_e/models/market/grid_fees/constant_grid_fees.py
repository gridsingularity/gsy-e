...
class ConstantGridFees(BaseClassGridFees):
    ...
    def calculate_original_trade_rate_from_clearing_rate(
        original_bid_rate: Decimal, propagated_bid_rate: Decimal, clearing_rate: Decimal
    ) -> Decimal:
    ...
    def propagate_original_offer_info_on_bid_trade(
        self, trade_original_info: TradeBidOfferInfo, ignore_fees=False
    ):
        grid_fee_rate = self.grid_fee_rate if not ignore_fees else Decimal(0.0)
        offer_rate = Decimal(trade_original_info.propagated_offer_rate) + grid_fee_rate
        trade_offer_info = TradeBidOfferInfo(
    ...
    def calculate_trade_price_and_fees(
        ...
        bid_rate = Decimal(trade_bid_info.propagated_bid_rate)
