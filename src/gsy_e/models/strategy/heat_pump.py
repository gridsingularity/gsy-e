...
# from gsy_e.models.strategy.energy_parameters.heatpump.pcm_heatpump import (
...
class MultipleTankHeatPumpStrategy(TradingStrategyBase):
    ...
    def __init__(
        ...
        # todo: add validation for max_capacity_kJ
...
class HeatPumpStrategy(MultipleTankHeatPumpStrategy):
    ...
    def __init__(
        ...
        preferred_buying_rate: float = ConstSettings.HeatPumpSettings.PREFERRED_BUYING_RATE,
        cop_model_type: COPModelType = COPModelType.UNIVERSAL,
        max_capacity_kJ: float = 6.0,  # todo: put default value somewhere
    ):
        tank_parameters = [
            TankParameters(
                ...
                max_capacity_kJ=max_capacity_kJ
            )
        ]
        super().__init__(
