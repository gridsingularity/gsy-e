...
class PCMTankState(TankStateBase):
    ...
    def __init__(
        ...
        self._max_capacity_kWh = tank_parameters.max_capacity_kJ
    ...
    def _get_condensor_temp_from_heat_demand_kWh(
        ...
        condensor_temp_C = self.get_htf_temp_C(
            time_slot
    ...
    def increase_tank_temp_from_heat_energy(self, heat_energy_kWh: float, time_slot: DateTime):
        ...
        # todo: on the water tank side, we only add to the _temp_increase_K,
        #  but here we actually charge
    ...
    def decrease_tank_temp_from_heat_energy(self, heat_energy_kWh: float, time_slot: DateTime):
        ...
        # todo: on the water tank side, we only add to the _temp_decrease_K,
        #  but here we actually discharge
    ...
    def get_min_heat_energy_consumption_kWh(self, time_slot):
        return self._get_maximum_available_storage_energy_kWh(time_slot)

    def get_max_heat_energy_consumption_kWh(self, time_slot):
        deltaT = self.max_storage_temp_C - self.get_htf_temp_C(time_slot)
        assert deltaT > 0, (
            f"the tank temperature already exceeded its limit "
            f"{self.get_htf_temp_C(time_slot)}"
        )

        maximum_heat_demand_kWh = self._get_heat_demand_kWh_from_deltaT(deltaT)
        maximum_soc_energy_kWh = self._get_maximum_available_storage_energy_kWh(
            time_slot
        )
        maximum_heat_energy_kWh = min(maximum_heat_demand_kWh, maximum_soc_energy_kWh)
        return maximum_heat_energy_kWh
