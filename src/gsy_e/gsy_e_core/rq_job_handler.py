...
from pendulum import duration, instance, now, Duration
...
class SimulationJobHandler:
    ...
    def launch_simulation_from_rq_job(
        ...
        try:
            ...
            settings = cls._adapt_settings(settings)

            if events is not None:
                events = ast.literal_eval(events)

            cls._configure_constants_constsettings(scenario, settings, connect_to_profiles_db)
            ...
            past_slots_sim_state = cls._handle_scm_past_slots_simulation_run(
                scenario,
                settings,
                events,
            ...
            config = cls._create_config_settings_object(
        ...
        # pylint: disable=broad-except
            ...
            logger.error(
                "Error on jobId, %s, configuration id: %s",
                job_id,
                gsy_e.constants.CONFIGURATION_ID,
            )
    ...
    def _configure_constants_constsettings(
        ...
        if settings.get("scm"):
            ...
            ConstSettings.SCMSettings.SELF_CONSUMPTION_TYPE = settings["scm"][
    ...
    def _handle_scm_past_slots_simulation_run(
        ...
        # We are running SCM Canary Networks with some days of delay compared to realtime in order
        # to compensate for delays in transmission of the asset measurements.
        # Adding 4 hours of extra time to the SCM past slots simulation duration, in order to
        # compensate for the runtime of the SCM past slots simulation and to not have any results
        # gaps after this simulation run and the following Canary Network launch.
        config.end_date = (
        ...
        simulation_state = cls._simulation_class()(
            setup_module_name=scenario_name,
            simulation_config=config,
            simulation_events=events,
            redis_job_id=job_id,
    ...
    def run_simulation(
        ...
        try:
            redis_job_id = (
                redis_job_id
                if not saved_sim_state
                else saved_sim_state["general"]["simulation_id"]
            )

            simulation = cls._simulation_class()(
