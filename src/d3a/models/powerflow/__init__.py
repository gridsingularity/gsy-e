...
class PowerFlow:
    ...
    def _grid_forming(self, area):
        ...
        if area.strategy is None:
            ...
            area.pv_device = \
                self._add_generation_device(area, area.strategy.max_panel_power_W * 1e-3)
        elif isinstance(area.strategy, FinitePowerPlant):
            area.power_plant = \
                self._add_generation_device(area, area.strategy.max_available_power_kW)
    ...
    def _add_generation_device(self, area, peak_power_kw):
        """
        ...
        :return: It will add a generating device to the specified area node
        """
