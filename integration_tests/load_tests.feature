Feature: Load Tests
  ...
  Scenario: DefinedLoadStrategy follows the profile provided by the user as dict
    ...
    When we run the simulation with setup file strategy_tests.user_profile_load_dict and parameters [24, 60, 60]
  ...
  Scenario: DefinedLoadStrategy trades energy based on csv provided profile
    ...
    And gsy_e is installed
    When we run the simulation with setup file strategy_tests.user_profile_load_csv and parameters [24, 15, 15]
  ...
  Scenario: DefinedLoadStrategy trades energy based on a multiday csv profile
    ...
    And gsy_e is installed
    When we run a multi-day gsy_e simulation with strategy_tests.user_profile_load_csv_multiday [2019-01-01, 48, 15, 15]
  ...
  Scenario: LoadHoursStrategy buys energy in the rate range provided by the user as dict profile
    ...
    When we run the simulation with setup file strategy_tests.user_rate_profile_load_dict and parameters [24, 30, 30]
