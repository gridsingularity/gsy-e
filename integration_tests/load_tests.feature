Feature: Load Tests
  ...
  Scenario: DefinedLoadStrategy follows the profile provided by the user as dict
    ...
    When we run the d3a simulation with strategy_tests.user_profile_load_dict [None, 24, 15, 15]
  ...
  Scenario: DefinedLoadStrategy trades energy based on csv provided profile
    ...
    And d3a is installed
    When we run the d3a simulation with strategy_tests.user_profile_load_csv [None, 24, 15, 15]
  ...
  Scenario: DefinedLoadStrategy trades energy based on a multiday csv profile
    ...
    And d3a is installed
    When we run the d3a simulation with strategy_tests.user_profile_load_csv_multiday [2019-01-01, 24, 15, 15]
  ...
  Scenario: LoadHoursStrategy buys energy in the rate range provided by the user as dict profile
    ...
    And d3a is installed
    When we run the simulation with setup file strategy_tests.user_rate_profile_load_dict and parameters [None, 24, 15, 15, 1, 4]
  ...
  Scenario: Custom Load strategy works as expected
     ...
     And d3a is installed
     When we run the simulation with setup file jira.d3asim_638_custom_load and parameters [None, 24, 15, 15, 0, 4]
