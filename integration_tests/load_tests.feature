Feature: Load Tests
  ...
  Scenario: LoadHoursStrategy buys energy in the rate range provided by the user as dict profile
    ...
    And d3a is installed
    When we run the d3a simulation on console with strategy_tests.user_rate_profile_load_dict for 24 hrs slot_length: 30m, tick_length: 30s and markets: 4
  ...
  Scenario: Custom Load strategy works as expected
     ...
     And d3a is installed
     When we run the d3a simulation on console with jira.d3asim_638_custom_load for 24 hrs slot_length: 60m, tick_length: 60s and markets: 4
