Feature: Run integration tests
  ...
  Scenario: Grid fees are calculated based on the clearing rate for pay as clear while dispatching top to bottom
     Given we have a scenario named non_compounded_grid_fees
     And d3a is installed
  ...
  Scenario: Grid fees are calculated based on the clearing rate for pay as clear while dispatching bottom to top
     Given we have a scenario named non_compounded_grid_fees
     And d3a is installed
     And d3a uses an two-sided pay-as-clear market
     And d3a dispatches events from bottom to top
     When we run the simulation with setup file non_compounded_grid_fees and parameters [24, 60, 60, 0, 1]
     Then trades on House 1 clear with 30 or 24 cents/kWh
     Then trades on Neighborhood 1 clear with 30 or 24 cents/kWh
     Then trades on Grid clear with 28.75 or 22.90909090 cents/kWh
     Then trades on Neighborhood 2 clear with 26.25 or 20.86956521 cents/kWh
     Then trades on House 2 clear with 25 or 20 cents/kWh
