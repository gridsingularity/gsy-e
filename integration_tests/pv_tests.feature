Feature: PV Tests
  ...
  Scenario: PV limits the minimum selling rate
     ...
     When we run the d3a simulation with strategy_tests.pv_final_selling_rate [None, 24, 15, 15]
  ...
  Scenario: PV RISK based price decrease
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.pv_risk_based_price_decrease [None, 24, 15, 15]
  ...
  Scenario: PV CONSTANT based price decrease
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.pv_const_price_decrease [None, 24, 15, 15]
     Then the PV strategy decrease its sold/unsold offers price as expected
  ...
  Scenario: PV can use the market maker rate as the initial rate for every market slot
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.pv_initial_pv_rate_option [None, 24, 15, 15]
  ...
  Scenario: UserProfile PV follows the profile provided by the user as dict
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.user_profile_pv_dict [None, 24, 15, 15]
  ...
  Scenario: Predefined PV follows the profile provided by the user as csv
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.user_profile_pv_csv [None, 24, 15, 15]
  ...
  Scenario: Predefined PV follows the profile provided by the user
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.predefined_pv_risk [None, 24, 15, 15]
  ...
  Scenario: Custom PV strategy works as expected
    ...
    When we run the d3a simulation with jira.d3asim_640_custom_pv [None, 24, 15, 30]
  ...
  Scenario: PV max_panel_power_W argument changes power of a single PV panel
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.pv_max_panel_output [None, 24, 15, 15]
