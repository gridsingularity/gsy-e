...
def follow_device_registry_energy_rate(context, device_name):
    ...
    assert all((trade.order.energy_rate) - negative_energy_rate
               < FLOATING_POINT_TOLERANCE
               for ii, device in enumerate([house1, house2, context.simulation.area])
               for market in device.past_balancing_markets
               for trade in market.trades if trade.order.energy < 0)

    assert all(trade.order.energy_rate - positive_energy_rate
               < FLOATING_POINT_TOLERANCE
               for ii, device in enumerate([house1, house2, context.simulation.area])
               for market in device.past_balancing_markets
               for trade in market.trades if trade.order.energy > 0)
...
def match_balancing_trades_to_spot_trades(context, functor):
    ...
    for area_object in [house1, house2]:
        for market in area_object.past_balancing_markets:
            ...
            spot_energy = past_market.trades[0].order.energy
            assert functor(isclose(abs(trade.order.energy),
                                   abs(spot_energy) *
...
def follow_device_registry_energy_rate_or(context, device1, device2):
    ...
    assert all(
        int(round(trade.order.energy_rate, 2)) in negative_energy_rate
        for device in [house1, house2, context.simulation.area]
        for market in device.past_balancing_markets
        for trade in market.trades if trade.order.energy < 0)

    assert all(
        int(round(trade.order.energy_rate, 2)) in positive_energy_rate
        for device in [house1, house2, context.simulation.area]
        for market in device.past_balancing_markets
        for trade in market.trades if trade.order.energy > 0)
