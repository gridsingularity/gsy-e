...
@given('gsy_e uses an {market_type} market')
...
@given('gsy_e dispatches events from top to bottom')
def dispatch_top_bottom(context):
    import gsy_e.constants
    gsy_e.constants.DISPATCH_EVENTS_BOTTOM_TO_TOP = False


@given('gsy_e dispatches events from bottom to top')
...
@when('we run the gsy_e simulation on console with {scenario} for {hours} hrs')
...
@when('we run the gsy_e simulation with compare-alt-pricing flag with {scenario}')
...
@when('we run the gsy_e simulation with cloud_coverage [{cloud_coverage}] and {scenario}')
...
def past_markets_not_in_memory(context):
    # gsy_e has to be set to publish the full results:
...
@when('we run a multi-day gsy_e simulation with {scenario} [{start_date}, {total_duration}, '
      '{slot_length}, {tick_length}]')
