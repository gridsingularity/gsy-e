...
def step_impl(context):
    ...
    assert all(
        [trade.trade_rate < 16.99 for trade in storage_trades])
...
def pv_produces_same_amount_of_energy_day(context):
    ...
    for base_market in house2.past_markets:
        ...
        same_time_markets_energy = [sum(trade.traded_energy
                                        for trade in market.trades
...
def _assert_hours_of_day(area, device):
    for market in area.past_markets:
        total_energy = sum(trade.traded_energy
...
def trades_on_all_markets_max_load_rate(context):
    ...
    for market in grid.past_markets:
        ...
        assert all(
            isclose(trade.trade_rate, max_rate[market.time_slot])
            for trade in market.trades)
