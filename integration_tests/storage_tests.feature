Feature: Storage tests
  ...
  Scenario: Storage break even range profile
     ...
     When we run the d3a simulation with strategy_tests.storage_strategy_break_even_range [None, 24, 15, 15]
  ...
  Scenario: Storage sell offers decrease
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.ess_sell_offer_decrease_based_on_risk_or_rate [None, 24, 15, 15]
  ...
  Scenario: Storage break even hourly profile
     ...
     And d3a is installed
     When we run the d3a simulation with strategy_tests.storage_strategy_break_even_hourly [None, 24, 15, 15]
  ...
  Scenario: Storage capacity dependant sell price
   ...
   And d3a is installed
   When we run the d3a simulation with strategy_tests.ess_capacity_based_sell_offer [None, 24, 15, 15]
  ...
  Scenario: Custom storage works as expected
   ...
   And d3a is installed
   When we run the d3a simulation with jira.d3asim_639_custom_storage [None, 24, 15, 30]
  ...
  Scenario: Storage buys and offers energy in the same market slot
   ...
   And d3a is installed
   When we run the d3a simulation with strategy_tests.storage_buys_and_offers [None, 24, 60, 60]
